...
docker exec -i "$fipe_preview_container" psql -U postgres -v ON_ERROR_STOP=1 < initial.sql
...
- [src/index.ts](src/index.ts) defines `classify`, `--dry-run`, and `-n`. The preview returns before
