...
| Publish new months in order and retry unfinished view updates and backups.                                   | `refresh [--backup]`                       | [Refresh](src/crawler/refresh.ts), [database](src/db/repository.ts), [SQL](initial.sql)                  | [Unit tests](src/crawler/refresh.test.ts) cover rules and retries. [Integration tests](src/crawler/refresh.integration.test.ts) check the publication price floor. [SQL tests](src/db/latest-prices.test.ts) check published-only prices in the schema and migration. |
...
PostgreSQL tests. They check `latest_prices` using the tracked SQL and run
[`runRefresh`](src/crawler/refresh.integration.test.ts) with fixed crawl data: below the price
floor, `published_at` stays null; at the floor, it gets set. The previous month's marker stays
unchanged in both cases. Live crawling, lock contention, AI output, and backups still need separate
checks. The restore drill is available for operators; this map doesn't claim it has been run.
